...
SEED_CUSTOMER_SMF = "Schwarzwaldmilch GmbH Freiburg"
SEED_PROJECTS_SMF = [
    ("Montage Frucht-/Kräuterdosieranlage Rahmmischer Frischkäserei", SEED_CUSTOMER_SMF, "Projekt 23670"),
    ("Installation Druckluftprojekt (Adsorptionstrockner)",           SEED_CUSTOMER_SMF, "Projekt 23650"),
    ("Schließen Rohrdurchführungen Bühne über Kontifill / Spritzschutz Ventilknotenwanne", SEED_CUSTOMER_SMF, "153203"),
    ("Rückbau Fruchtmischanlage Hamba-A",                              SEED_CUSTOMER_SMF, "153201"),
    ("Montage Löschwasserleitung Bereich Expedition",                  SEED_CUSTOMER_SMF, "110001"),
    ("Montage Winkelbleche Heberschächte (8 Stck.)",                   SEED_CUSTOMER_SMF, "Projekt 23610"),
    ("Montage pH-Armatur Abwasseranlage",                              SEED_CUSTOMER_SMF, "110017"),
    ("Umbau Abwasserpumpen Rechen",                                    SEED_CUSTOMER_SMF, "110017"),
]
...
def init_db():
    with db() as c:
        ...
        # Schwarzwaldmilch-Projekte seeden / reaktivieren
        for name, customer, cc in SEED_PROJECTS_SMF:
            row = c.execute(
                "SELECT id, active FROM projects WHERE name=? AND customer=? AND cost_center=?",
                (name, customer, cc),
            ).fetchone()
            if row:
                if not row["active"]:
                    c.execute("UPDATE projects SET active=1 WHERE id=?", (row["id"],))
            else:
                c.execute(
        ...
        # Platzhalter und Demo-Projekte deaktivieren (falls vorhanden)
        c.execute(
            "UPDATE projects SET active=0 WHERE name IN ('Neubau Halle B','Sanierung Dach','Montage Lüftung','Heizung Austausch')"
        )
        c.execute("UPDATE projects SET active=0 WHERE name='Maßnahme'")

        LOG.info("Schwarzwaldmilch-Projekte seeding beendet")

        # Abwesenheit-Projekt immer vorhanden, aber nicht auswählbar
        absence = c.execute("SELECT id, active FROM projects WHERE name=?", (ABSENCE_PROJECT_NAME,)).fetchone()
        if not absence:
            c.execute(
                "INSERT INTO projects (name,customer,cost_center,active) VALUES (?,?,?,0)",
                (ABSENCE_PROJECT_NAME, "Intern", "ABW"),
            )
        elif absence["active"]:
            c.execute("UPDATE projects SET active=0 WHERE id=?", (absence["id"],))
...
async def _prompt_project(reply_func: Callable, ctx):
    ps = get_projects()
    if not ps:
...
def export_customer_week(project_id: int, kw: int, year: int) -> Path:
    ...
    for e in entries:
        ...
        if not bucket:
            ...
            grouped[d][key] = bucket
        else:
            bucket["stime"] = min(bucket["stime"], e["stime"])
            bucket["etime"] = max(bucket["etime"], e["etime"])
            bucket["brk"] += int(e["brk"] or 0)
            bucket["hours"] = round(bucket["hours"] + float(e["hours"] or 0), 2)
    ...
    for day_index, (start_row, end_row) in enumerate(blocks):
        ...
        items = list(grouped.get(current_day, {}).values())
        items = _fit_customer_rows(items, capacity)
...
def export_internal_year(user_id: int, year: int) -> Path:
    ...
    ws_year = wb["2026"] if "2026" in wb.sheetnames else wb[wb.sheetnames[0]]
    ...
    original_month_names = list(MONTH_NAMES_DE.values())
    for month in range(1, 13):
        source_name = original_month_names[month - 1]
        ws = wb[source_name]
        ws["A1"] = month
        ws["D1"] = year
        ...
        while current.month == month:
            ...
            for idx, value in enumerate(slots, start=3):
            ...
            if vac_hours:
                ws.cell(row, 9).value = round(vac_hours / daily_target, 2)
            if sick_hours:
                ws.cell(row, 10).value = round(sick_hours / daily_target, 2)
...
async def cmd_exportkunde(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not customers:
        ...
        return ConversationHandler.END
    kb = [[InlineKeyboardButton(customer, callback_data=f"ec_{i}")] for i, customer in enumerate(customers)]
    ctx.user_data["_ek_customers"] = customers
    await update.message.reply_text(t(L(ctx), "pick_customer"), reply_markup=InlineKeyboardMarkup(kb))
...
async def ek_customer(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(p["name"], callback_data=f"epj_{p['id']}")] for p in projects]
    await q.edit_message_text(t(L(ctx), "pick_project_export"), reply_markup=InlineKeyboardMarkup(kb))
    return S_EK_PROJECT
...
async def cmd_exportintern(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if FOREMAN(ctx):
        users = get_users()
        kb = [[InlineKeyboardButton(t(L(ctx), "self_export"), callback_data="iu_self")]]
        kb.extend([[InlineKeyboardButton(f"👤 {u['name']}", callback_data=f"iu_{u['id']}")] for u in users if u["id"] != UID(ctx)])
        await update.message.reply_text(t(L(ctx), "pick_export_user"), reply_markup=InlineKeyboardMarkup(kb))
        return S_EI_TARGET
...
def main():
    ...
    app.add_handler(
        ConversationHandler(
            ...
            states={
                S_EK_CUSTOMER: [CallbackQueryHandler(ek_customer, pattern=r"^ec_\d+$")],
                S_EK_PROJECT: [CallbackQueryHandler(ek_project, pattern=r"^epj_\d+$")],
    ...
    app.add_handler(
        ConversationHandler(
            ...
            states={
                S_EI_TARGET: [CallbackQueryHandler(ei_target, pattern=r"^iu_")],
